...
if ($action === 'get' || $action === 'get_all') {
    // Lấy tất cả đơn hàng
    $sql = "SELECT dh.*, kh.ten as ten_khachhang FROM donhang dh JOIN khachhang kh ON dh.id_khachhang = kh.id_khachhang";
    $result = mysqli_query($conn, $sql);
    if (!$result) sendResponse(false, 'Lỗi truy vấn SQL', mysqli_error($conn));
    $data = [];
